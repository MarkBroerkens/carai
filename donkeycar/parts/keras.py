...
class KerasLinear(KerasPilot):
    def __init__(self, model=None, num_outputs=None, *args, **kwargs):
        ...
        if model:
            print("Linear NN with given model")
            self.model = model
        #elif num_outputs is not None:
         #   print("Linear NN with n linear")
         #   self.model = default_n_linear(num_outputs)
        else:
            print("Linear NN with linear")
            #self.model = default_linear()
            #self.model = marks_linear()
            self.model = marks_nvidia_linear()
...
def default_linear():
    img_in = Input(shape=(120, 160, 3), name='img_in')
    x = img_in
    #x = Cropping2D(cropping=((40, 0), (0, 0)))(x)  # trim 52 pixels off top
    x = Convolution2D(24, (5, 5), strides=(2, 2), activation='relu')(x)
    x = Convolution2D(32, (5, 5), strides=(2, 2), activation='relu')(x)
    x = Convolution2D(64, (5, 5), strides=(2, 2), activation='relu')(x)
    x = Convolution2D(64, (3, 3), strides=(2, 2), activation='relu')(x)
